...
export default function InboxPage() {
  ...
  const [selectedIncomingLetter, setSelectedIncomingLetter] =
    useState<any>(null);
  ...
  const [writingRecipient, setWritingRecipient] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const handleUnlockLetter = async () => {
    ...
    if (res.success && res.letter) {
      toast.success(
        "Mektubunuz başarıyla açıldı! İstediğiniz zaman ücretsiz olarak görüntüleyebilirsiniz.",
      );
      // Mektubu güncelleyelim (isRead=true ve images dolacak)
      const updatedIncomingLetters = incomingLetters.map((l) =>
        l.id === unlockingLetter.id
          ? { ...l, isRead: true, images: res.letter?.images || [] }
          : l,
      );
      ...
      setSelectedIncomingLetter({
        ...unlockingLetter,
        isRead: true,
        images: res.letter?.images || [],
      });
      setIsIncomingModalOpen(true);
    } else {
      ...
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-playfair font-bold text-ink">
              Gelen Mektuplarım
            </h1>
          </div>
          ...
          <div className="flex flex-col items-end gap-3">
            <div className="flex items-center gap-4 bg-white/50 backdrop-blur-sm p-4 rounded-[2rem] border border-paper-dark shadow-sm">
              ...
              <button
                ...
                disabled={isToggling}
                className={`w-14 h-8 rounded-full relative transition-all duration-500 shadow-inner ${
                  notificationsEnabled ? "bg-seal" : "bg-paper-dark"
                }`}
              >
                <div
                  className={`absolute top-1 w-6 h-6 rounded-full bg-white shadow-md transition-all duration-500 flex items-center justify-center ${
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-ink-light font-bold font-kurale">
              Mektuplarınız getiriliyor...
            </p>
          </div>
        ) : allItems.length === 0 ? (
          <motion.div
          ...
          >
            ...
            <h3 className="text-2xl font-playfair font-bold text-ink">
              Henüz Gelen Mektubunuz Yok
            </h3>
            <p className="text-ink-light max-w-md mx-auto font-medium">
              Size gönderilen mektuplar burada görünecektir. Adresinizi
              paylaşarak sevdiklerinizden mektup alabilirsiniz.
            </p>
          </motion.div>
        ) : (
                  ...
                  {item._type === "incoming" ? (
                    ...
                    >
                      ...
                      <div className="space-y-4 flex-1">
                        <div>
                          <p className="text-[10px] font-black text-ink-light uppercase tracking-widest mb-1">
                            Gönderici
                          </p>
                      ...
                      <div className="mt-8 pt-6 border-t border-seal/20 flex items-center justify-between text-seal overflow-hidden">
                        <span className="text-xs font-black uppercase tracking-widest group-hover:translate-x-1 transition-transform duration-300">
                          Mektubu Oku
                        </span>
                        <ArrowRight
                          size={18}
                          className="translate-x-8 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300"
                        />
                      </div>
                    </div>
                  ) : (
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="flex items-center gap-2 text-ink-light">
                            ...
                            <span className="text-xs font-bold truncate">
                              {item.receiverCity ||
                                item.data?.address?.receiverCity ||
                                "-"}
                            </span>
                          </div>
                        </div>
            ...
            >
              <div className="p-6 border-b border-paper-dark flex justify-between items-center bg-paper-light">
                ...
                <button
                  ...
                  className="text-ink-light hover:text-ink transition-colors p-1 bg-paper border border-paper-dark rounded-full shadow-sm hover:shadow-md disabled:opacity-50"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-5 text-center">
                <div className="w-16 h-16 bg-seal/10 rounded-full flex items-center justify-center mx-auto mb-2 relative">
                  ...
                  <span className="absolute -top-1 -right-1 bg-white text-seal text-xs font-bold px-1.5 py-0.5 rounded shadow-sm border border-seal/20">
                    +{unlockingLetter.imageCount || 1}
                  </span>
                </div>
                <p className="text-sm text-ink-light leading-relaxed">
                  Adınıza gelen fiziksel mektubu dijital olarak okumak ve kalıcı
                  olarak kilit açmak üzeresiniz.
                </p>
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
                ...
                {isSearching ? (
                  ...
                  searchResults.map((user) => (
                    ...
                    >
                      ...
                      <ArrowRight
                        size={18}
                        className="ml-auto text-ink-light opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all"
                      />
                    </button>
                  ))
              ...
              <p className="mt-8 text-[11px] text-ink-light text-center leading-relaxed font-medium">
                Mektup arkadaşınızın sistemimizde kayıtlı olması gerekmektedir.{" "}
                <br />
