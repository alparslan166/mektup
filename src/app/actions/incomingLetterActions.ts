...
export async function createIncomingLetter(
  ...
  try {
    ...
    if (user?.email) {
      await sendIncomingLetterNotificationEmail(
        user.email,
...
// User: Mark an incoming letter as read after payment confirmation
export async function markIncomingLetterAsRead(letterId: string) {
  ...
  try {
    ...
    // Ödeme sağlayıcısı entegrasyonu sonraki adımda eklenecek.
    // Şimdilik kullanıcı arayüzündeki ödeme onayı sonrası kilit açma işlemi yapıyoruz.
    ...
    // Eğer update edilemediyse (başka bir istek bizden önce update etmiş demektir)
    if (updateResult.count === 0) {
      return { success: true, price: openPrice };
    }
    ...
    return { success: true, letter: updatedLetter, price: openPrice };
  } catch (error) {
