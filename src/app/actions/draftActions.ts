...
function getDraftExpiryDate() {
  return new Date(Date.now() - DRAFT_RETENTION_DAYS * 24 * 60 * 60 * 1000);
}

async function cleanupExpiredDraftsForUser(userId: string) {
  await prisma.draft.deleteMany({
    where: {
      userId,
      updatedAt: {
        lt: getDraftExpiryDate(),
      },
    },
  });
}
...
export async function saveDraft(data: any, draftId?: string | null) {
  try {
    ...
    await cleanupExpiredDraftsForUser(user.id);
