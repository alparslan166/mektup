...
export default function DraftListClient({ drafts }: DraftClientProps) {
  ...
  const handleResume = (draft: any) => {
    ...
    hydrateStore({
      draftId: draft.id,
      orderNumber: draft.data?.orderNumber || null,
  ...
  if (drafts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed border-wood/20 rounded-2xl bg-paper/50 mt-8">
        ...
        <h3 className="text-xl font-bold text-wood-dark mb-2 font-playfair">
          Henüz bir taslağınız yok
        </h3>
        <p className="text-ink-light mb-6">
          Yarım bıraktığınız mektuplar otomatik olarak burada görünür.
        </p>
        <button
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        {drafts.map((draft) => {
          ...
          const previewText =
            rawContent.replace(/<[^>]+>/g, "").slice(0, 100) + "...";
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-2">
                <div
                  className={`p-2 rounded-xl border ${isDM ? "bg-seal/10 text-seal border-seal/20" : "bg-paper text-wood border-wood/20"}`}
                >
                ...
                <h4 className="font-playfair text-xl font-bold text-wood-dark">
                  {draft.data?.address?.receiverName
                    ? isDM
                      ? `Dijital Mektup: ${draft.data.address.receiverName}`
                      : `Alıcı: ${draft.data.address.receiverName}`
                    : "İsimsiz Mektup"}
                </h4>
              </div>
          ...
          >
            ...
            >
              ...
              <p className="text-center text-ink-light text-sm mb-8 leading-relaxed">
                Bu taslağı silmek istediğinize emin misiniz? Bu işlem geri
                alınamaz.
              </p>
