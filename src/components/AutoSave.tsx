...
export default function AutoSave() {
  const { draftId, setDraftId, orderNumber, letter, extras, address } =
    useLetterStore();
  ...
  useEffect(() => {
    ...
    saveTimeout.current = setTimeout(async () => {
      const dataToSave = { orderNumber, letter, extras, address };
  ...
  }, [orderNumber, letter, extras, address, status, draftId, setDraftId]);
      ...
      <div className="bg-paper shadow-lg border border-wood/20 rounded-full px-4 py-2 flex items-center gap-2 animate-in fade-in slide-in-from-bottom-4 duration-300">
        {saveStatus === "saving" && (
          <>
            ...
            <span className="text-[11px] font-bold text-wood-dark tracking-tighter uppercase">
              Kaydediliyor...
            </span>
          </>
        )}
        {saveStatus === "saved" && (
          <>
            ...
            <span className="text-[11px] font-bold text-emerald-600 tracking-tighter uppercase">
              Kaydedildi
            </span>
          </>
        )}
        {saveStatus === "error" && (
          <>
            ...
            <span className="text-[11px] font-bold text-red-600 tracking-tighter uppercase">
              Hata! Kaydedilemedi
            </span>
          </>
        )}
        {saveStatus === "idle" && (
          <>
            ...
            <span className="text-[11px] font-bold text-wood-dark/40 tracking-tighter uppercase">
              Bulut Senkronize
            </span>
          </>
        )}
