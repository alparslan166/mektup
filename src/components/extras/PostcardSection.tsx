    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4 mb-8 animate-in fade-in duration-700">
        ...
        <p className="text-ink-light text-sm max-w-xl mx-auto leading-relaxed">
          ...
          <span className="text-seal font-medium">
            10x15 cm boyutunda, yüksek kaliteli fotoğraf baskısı.
          </span>
        </p>
          ...
          <p className="text-xs text-ink-light space-y-1">
            <span className="block">
              ...
              <strong className="text-wood-dark">
                {postcardCreditPrice} TL
              </strong>{" "}
              değerindedir.
            </span>
            <span className="block">
              • <strong>3. Kartpostalda:</strong> %20 İndirim fırsatı!
            </span>
            <span className="block">
              • <strong>5 Kartpostal seçerseniz:</strong> 1 Tanesi Bizden
              Hediye!
            </span>
            <span className="block">
              • <strong>10 Kartpostal seçerseniz:</strong> Tam 2 Tanesi Hediye!
            </span>
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {postcardCategories.map((category) => (
          <div
            key={category.id}
            className="border border-paper-dark rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-all duration-300"
          >
            {/* Accordion Header */}
            <button
              onClick={() => toggleCategory(category.id)}
              className={
                "w-full flex items-center py-5 px-6 gap-4 transition-all text-left " +
                (openCategories[category.id]
                  ? "bg-slate-50 border-b border-paper-dark/50"
                  : "hover:bg-slate-50")
              }
            >
              ...
              <ChevronDown
                size={22}
                className={
                  "text-seal transition-transform duration-300 " +
                  ...
                  {category.items.map((card) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={
                            "relative aspect-square w-full rounded-3xl overflow-hidden shadow-sm cursor-pointer group transition-all duration-500 hover:shadow-xl hover:-translate-y-1 bg-white border-2 " +
                            (isSelected
                              ? "border-seal ring-4 ring-seal/10"
                              : "border-paper-dark")
                          }
                        ...
                        >
                          ...
                          <div
                            className={
                              "absolute inset-0 bg-seal/10 pointer-events-none transition-opacity duration-300 " +
                        ...
                        <p className="mt-3 text-center text-xs font-bold text-ink-light uppercase tracking-widest">
                          {card.title}
                        </p>
                      </div>
                    );
      ...
      <div className="text-center space-y-6 mb-8 bg-paper-dark/5 p-8 rounded-3xl border border-paper-dark/50 shadow-inner">
        <div className="space-y-2">
          ...
          <p className="text-ink-light text-sm max-w-xl mx-auto italic">
            En az <span className="font-bold text-seal not-italic">3 adet</span>{" "}
            fotoğraf, belge veya kartpostal eklemeniz halinde ücretsiz!
          </p>
        </div>
        ...
        <div className="flex flex-col items-center gap-4">
          <button
            ...
            className={
              "flex items-center gap-3 px-10 py-4 rounded-full border-2 font-black tracking-widest uppercase text-sm transition-all duration-300 active:scale-95 shadow-md " +
              (includeCalendar
                ? "border-seal bg-seal text-white hover:bg-seal-dark"
                : "border-paper-dark bg-white text-ink-light hover:border-seal/50 hover:text-seal")
            }
          >
            {includeCalendar ? (
            ...
            >
              ...
              <div className="p-8 sm:p-10 flex flex-col gap-8 bg-paper-light">
                <div className="space-y-2 text-center">
                  <h4 className="font-playfair text-3xl font-black text-ink uppercase tracking-wider">
                    {previewCard.title}
                  </h4>
                  <p className="text-sm text-ink-light font-medium tracking-wide">
                    10x15 CM YÜKSEK KALİTELİ KARTPOSTAL BASKISI
                  </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <button
                    ...
                    className={
                      "flex-1 px-8 py-4 rounded-full font-black tracking-widest uppercase text-xs transition-all active:scale-95 shadow-xl flex items-center justify-center gap-2 " +
                      (postcards.includes(previewCard.id)
                        ? "bg-red-500 text-white hover:bg-red-600 shadow-red-500/20"
                        : "bg-seal text-white hover:bg-seal-dark shadow-seal/20")
                    }
                  >
                    {postcards.includes(previewCard.id) ? (
                      <>SEÇİMİ KALDIR</>
