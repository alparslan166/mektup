...
export default function UploadSection() {
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      toast.error(
        "Dosya yüklenirken bir hata oluştu: " + (err as Error).message,
        { id: loadingToast },
      );
    } finally {
    ...
    <div className="space-y-6">
      <div className="text-center space-y-2 mb-8">
        <h3 className="font-playfair text-2xl font-bold text-wood-dark flex items-center justify-center gap-2">
          <span className="text-seal">»</span> Fotoğraf & Belge{" "}
          <span className="text-seal">«</span>
        </h3>
        <p className="text-ink-light text-sm max-w-xl mx-auto">
          Yüklediğiniz fotoğraflar 10x15 özel kodak fotoğraf kağıdına, belgeler
          ise A4 kağıdına basılır. Galerinizden eklediğiniz görsel bir metin,
          evrak, dilekçe vb. ise lütfen bu görseli &quot;Belge Ekle&quot;
          alanından yükleyiniz.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-6">
        ...
        >
          {isUploading ? (
            <Loader2 size={40} className="animate-spin text-wood/30" />
          ) : (
            <ImageIcon
              size={40}
              className="text-wood-dark mb-4 group-hover:scale-110 transition-transform"
            />
          )}
          <span className="font-bold text-ink tracking-wide">FOTOĞRAF</span>
          <span className="font-bold text-ink tracking-wide">EKLE</span>
          {!isUploading && (
            <Plus size={16} className="absolute top-2 right-2 text-wood/40" />
          )}
        </button>
        ...
        >
          {isUploading ? (
            <Loader2 size={40} className="animate-spin text-ink-light/30" />
          ) : (
            <FileText
              size={40}
              className="text-ink mb-4 group-hover:scale-110 transition-transform"
            />
          )}
          ...
          {!isUploading && (
            <Plus
              size={16}
              className="absolute top-2 right-2 text-ink-light/40"
            />
          )}
        </button>
      </div>
      ...
      <div className="text-center pt-2 space-y-3">
        <p className="text-sm font-medium text-wood-dark">
          Toplamda en fazla <span className="font-bold text-seal">10</span> adet
          fotoğraf/belge ekleyebilirsiniz.
        </p>
        ...
        <div className="bg-seal/5 border border-seal/20 rounded-xl p-4 max-w-lg mx-auto shadow-sm">
          ...
          <p className="text-xs text-ink-light space-y-1">
            <span className="block">
              • Her fotoğraf{" "}
              <strong className="text-wood-dark">{photoCreditPrice} TL</strong>,
              belge{" "}
              <strong className="text-wood-dark">{docCreditPrice} TL</strong>{" "}
              değerindedir.
            </span>
            <span className="block">
              • <strong>3. Fotoğrafta:</strong> %20 İndirim fırsatı!
            </span>
            <span className="block">
              • <strong>5 Fotoğraf yüklerseniz:</strong> 1 Tanesi Bizden Hediye!
            </span>
            <span className="block">
              • <strong>10 Fotoğraf yüklerseniz:</strong> Tam 2 Tanesi Hediye!
            </span>
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 bg-paper-dark/10 border border-paper-dark rounded-lg min-h-[100px] p-4">
        {photoCount + docCount === 0 ? (
          ...
          <div className="space-y-4">
            {photoCount > 0 && (
              <div>
                <h4 className="text-sm font-bold text-wood-dark mb-2 px-2 border-b border-paper-dark pb-1">
                  Eklenen Fotoğraflar ({photoCount})
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
                  {extras.photos.map((file) => (
                    <div
                      key={file.id}
                      className="relative group bg-paper p-2 rounded border border-paper-dark shadow-sm"
                    >
            ...
            {docCount > 0 && (
              <div className={photoCount > 0 ? "pt-4" : ""}>
                <h4 className="text-sm font-bold text-wood-dark mb-2 px-2 border-b border-paper-dark pb-1">
                  Eklenen Belgeler ({docCount})
                </h4>
                <div className="flex flex-wrap gap-3">
                  {extras.documents.map((file) => (
                    <div
                      key={file.id}
                      className="flex items-center justify-between bg-paper p-2 pr-3 rounded border border-paper-dark shadow-sm min-w-[200px] group"
                    >
