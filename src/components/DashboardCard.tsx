...
export default function DashboardCard({ title, description, icon: Icon, href, color, delay = 0, onClick, className = "" }: DashboardCardProps) {
    const colorClasses: Record<string, string> = {
        ...
        red: "border-red-500/20 bg-red-50/95",
    };
    ...
    return (
        ...
        >
            <Link href={href} onClick={onClick} className="group block h-full">
                <div className={`h-full min-h-[160px] p-4 sm:p-8 rounded-2xl border-2 transition-all duration-300 group-hover:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] backdrop-blur-md shadow-xl ${colorClasses[color]} flex flex-col items-center justify-center text-center relative overflow-hidden ${className}`}>
                    {/* Top Accent Bar */}
