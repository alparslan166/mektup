    ...
    <header className="bg-transparent border-b border-paper/10 py-3 w-full z-50 relative">
      <div className="container mx-auto px-6 max-w-7xl flex justify-between items-center gap-4">
        ...
        <Link
          href="/"
          onClick={closeMenu}
          className="flex flex-col hover:opacity-90 transition-opacity shrink-0 -ml-4 lg:-ml-20"
        >
          <div className="flex items-center gap-1">
            ...
            <h1 className="font-playfair text-2xl lg:text-3xl font-bold tracking-widest text-paper drop-shadow-md">
              Mektuplaş
            </h1>
          </div>
          <span className="text-[10px] lg:text-[12px] font-semibold tracking-widest text-paper/70 mt-1 italic drop-shadow-sm">
        ...
        <nav className="text-[11px] lg:text-[13px] font-bold tracking-wider text-paper/90 gap-4 lg:gap-6 hidden md:flex items-center flex-1 justify-center">
          {(session?.user as any)?.role === "ADMIN" && (
            <button
              onClick={handleAdminMenusClick}
              className="cursor-pointer text-paper/90 hover:text-white hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.5)] transition-all flex flex-col items-center"
            >
              <svg
                width="24"
                height="24"
          ...
          {adminMenus ? (
            ...
            <>
              <Link
                href={
                  status === "unauthenticated" ? "/auth/login" : "/mektup-yaz"
                }
                onClick={resetStore}
                className="flex items-center gap-1.5 hover:text-white hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.5)] transition-all"
              >
              ...
              {status === "authenticated" && (
                <Link
                  href="/gelen-kutusu"
                  className="flex items-center gap-1.5 hover:text-white hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.5)] transition-all relative"
                >
                  <Inbox size={18} />
              ...
              <Link
                href="/kampanyalar"
                className="flex items-center gap-1.5 hover:text-white hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.5)] transition-all text-amber-200"
          ...
          {status === "authenticated" ? (
            <>
              ...
              {(session.user as any)?.role === "ADMIN" ? (
                <Link
                  href="/admin/mektuplar"
                  className="flex items-center gap-2 bg-seal/10 backdrop-blur-md text-seal border border-seal/20 hover:border-seal/40 hover:bg-seal/20 px-4 py-2 rounded-md font-bold text-[13px] transition-all shadow-sm"
                >
                  <ShieldCheck size={16} />
            ...
            <>
              <Link
                href="/auth/login"
                className="flex items-center gap-2 bg-paper text-wood-dark border border-paper hover:bg-paper/90 px-4 py-2 rounded-md font-bold text-[13px] transition-all shadow-md"
              >
                <LogIn size={16} />
                <span>Giriş Yap</span>
              </Link>
              <Link
                href="/auth/register"
                className="flex items-center gap-2 bg-seal text-paper border border-seal hover:bg-seal-hover px-4 py-2 rounded-md font-bold text-[13px] transition-all shadow-md"
              >
                <UserPlus size={16} />
                <span>Kayıt Ol</span>
              </Link>
            ...
            {adminMenus ? (
              ...
              <>
                ...
                <Link
                  href="/kampanyalar"
                  onClick={closeMenu}
                  className="flex items-center gap-4 py-2 border-b border-paper/10 text-amber-200 hover:text-white"
                >
                  <Tag size={22} />
                  <span>KAMPANYALAR</span>
                </Link>
              ...
              {status === "authenticated" ? (
                <>
                  {/* <Link
                    href="/cuzdan"
                    onClick={closeMenu}
                    className="flex items-center justify-center gap-2 bg-wood/10 text-wood border border-wood/30 px-6 py-4 rounded-xl font-bold transition-all shadow-sm"
                  >
                    <Wallet size={20} />
                  ...
                  {(session.user as any)?.role === "ADMIN" && (
                    <Link
                      href="/admin/mektuplar"
                      onClick={closeMenu}
                      className="flex items-center justify-center gap-2 bg-seal/10 text-seal border border-seal/20 px-6 py-4 rounded-xl font-bold transition-all shadow-sm"
                    >
                      <ShieldCheck size={20} />
                      <span>Admin Paneli</span>
                    </Link>
                ...
                <>
                  <Link
                    href="/auth/login"
                    onClick={closeMenu}
                    className="flex items-center justify-center gap-2 bg-paper text-wood-dark border border-paper px-6 py-4 rounded-xl font-bold transition-all shadow-md"
                  >
                    <LogIn size={20} />
                    <span>Giriş Yap</span>
                  </Link>
                  <Link
                    href="/auth/register"
                    onClick={closeMenu}
                    className="flex items-center justify-center gap-2 bg-seal text-paper border border-seal px-6 py-4 rounded-xl font-bold transition-all shadow-md"
                  >
                    <UserPlus size={20} />
                    <span>Kayıt Ol</span>
                  </Link>
