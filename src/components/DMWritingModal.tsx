...
export default function DMWritingModal({
  isOpen,
  onClose,
  recipientId,
  recipientName,
  ...
  const { letter, address, extras, updateAddress, resetStore } =
    useLetterStore();
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-10 custom-scrollbar">
              ...
              <div className="mb-8 bg-seal/5 border border-seal/20 rounded-2xl p-4 sm:p-6 flex flex-col md:flex-row items-center gap-4 shadow-sm animate-in slide-in-from-top duration-500">
                ...
                <div className="text-center md:text-left">
                  <h4 className="font-playfair text-lg font-bold text-wood-dark">
                    Dijital Teslimat Bilgilendirmesi
                  </h4>
                  <p className="text-ink-light text-[13px] leading-relaxed font-medium">
                    Bu mektup bir üye adına yazılmaktadır. Gönderdiğinizde
                    anında alıcının{" "}
                    <span className="text-seal font-bold">gelen kutusuna</span>{" "}
                    düşecektir.
                  </p>
                </div>
              </div>
                  ...
                  <div className="bg-white/50 border border-paper-dark rounded-2xl p-6 space-y-4 shadow-sm">
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1">
                        <label className="text-[11px] font-bold text-ink-light uppercase tracking-wider">
                          Ad Soyad
                        </label>
                        <input
                          type="text"
                          ...
                          onChange={(e) =>
                            updateAddress({ senderName: e.target.value })
                          }
                        />
                      </div>
                      <div className="space-y-1">
                    ...
                    {extras.photos.length < 3 && (
                      ...
                      >
                        {isUploadingPhoto ? (
                          <Loader2
                            size={24}
                            className="animate-spin text-wood/50 mb-2"
                          />
                        ) : (
                          <Plus
                            size={24}
                            className="text-wood-dark mb-2 group-hover:scale-110 transition-transform"
                          />
                        )}
                    ...
                    {extras.photos.length > 0 && (
                      <div className="grid grid-cols-2 gap-2 mt-4">
                        {extras.photos.map((file: any) => (
                          <div
                            key={file.id}
                            className="relative group bg-paper p-1 rounded border border-paper-dark shadow-sm"
                          >
            ...
            <div className="bg-paper shadow-[0_-4px_20px_rgba(0,0,0,0.05)] p-6 sm:px-10 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-paper-dark shrink-0">
              <div className="hidden sm:block">
                <p className="text-xs text-ink-light font-medium flex items-center gap-1">
                  ...
                  Bu işlem sonunda herhangi bir kargo ücreti
                  <span className="font-bold text-seal">yoktur.</span>
                </p>
              </div>
