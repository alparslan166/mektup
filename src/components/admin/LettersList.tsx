...
export default function LettersList({
  ...
  const handlePrintEnvelope = (letter: Letter) => {
    const address = letter.data?.address || {};
    const orderRef = letter.data?.orderNumber || letter.id;
    const logoUrl = `${window.location.origin}/images/kus-logo.png`;
    const barcodeSource = String(orderRef || "").replace(/[^A-Za-z0-9]/g, "");
    const normalizedBarcode = barcodeSource.padEnd(16, "0").slice(0, 24);

    let barcodeX = 0;
    const barcodeRects: string[] = [];

    for (let index = 0; index < normalizedBarcode.length; index += 1) {
      const code = normalizedBarcode.charCodeAt(index);
      const segments = [
        1 + (code % 3),
        1 + ((code >> 2) % 4),
        1 + ((code >> 4) % 3),
      ...
      segments.forEach((segmentWidth) => {
        barcodeRects.push(
          `<rect x="${barcodeX}" y="0" width="${segmentWidth}" height="56" fill="#111827" />`,
        );
        barcodeX += segmentWidth + 1;
      });

      barcodeX += 1;
    }

    const barcodeWidth = Math.max(barcodeX, 180);
    const barcodeSvg = `<svg viewBox="0 0 ${barcodeWidth} 56" preserveAspectRatio="none" aria-hidden="true">${barcodeRects.join("")}</svg>`;
    ...
    const senderName = escapeHtml(
      address.senderName || letter.senderName || "-",
    );
    const senderCity = escapeHtml(address.senderCity || "");
    const senderAddress = escapeHtml(address.senderAddress || "");
    const receiverName = escapeHtml(
      address.receiverName || letter.receiverName || "-",
    );
    const receiverCity = escapeHtml(
      address.receiverCity || letter.receiverCity || "",
    );
    const receiverAddress = escapeHtml(address.receiverAddress || "");
    const receiverPhone = escapeHtml(address.receiverPhone || "");
    const prisonName = escapeHtml(address.prisonName || "");
    const fatherName = escapeHtml(address.fatherName || "");
    const wardNumber = escapeHtml(address.wardNumber || "");
    const isPrison = Boolean(address.isPrison);

    const receiverBlock = isPrison
      ? `${receiverName}${fatherName ? ` / Baba Adı: ${fatherName}` : ""}${wardNumber ? ` / Koğuş: ${wardNumber}` : ""}<br/>${prisonName}<br/>${receiverAddress}<br/>${receiverCity}`
      : `${receiverName}<br/>${receiverAddress}<br/>${receiverCity}${receiverPhone ? `<br/>Tel: ${receiverPhone}` : ""}`;

    const printWindow = window.open("", "_blank", "width=900,height=650");
    if (!printWindow) {
      toast.error("Yazdırma penceresi açılamadı. Pop-up engelini kapatın.");
      return;
    }
    ...
    const html = `
      ...
      <html>
        <head>
          <meta charset="utf-8" />
          <title>Zarf Baskısı - ${escapeHtml(orderRef)}</title>
          <style>
            * { box-sizing: border-box; }
            body { margin: 0; background: #f1f5f9; font-family: Arial, sans-serif; }
            .page { width: 229mm; min-height: 162mm; margin: 10mm auto; background: #fff; border: 1px solid #cbd5e1; padding: 10mm; position: relative; }
            .top { display: flex; justify-content: space-between; align-items: flex-start; gap: 8mm; margin-bottom: 6mm; }
            .senderInfo { color: #0f172a; }
            .senderInfo h3 { margin: 0 0 2mm; font-size: 14px; font-weight: 800; letter-spacing: .04em; }
            .senderInfo p { margin: 0; line-height: 1.35; font-size: 10.5px; }
            .stamp { width: 28mm; height: 38mm; border: 1px dashed #64748b; border-radius: 2mm; overflow: hidden; background: #f8fafc; }
            .stamp img { width: 100%; height: 100%; object-fit: cover; }
            .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8mm; }
            .box { border: 2px solid #d1d5db; background: #f8fafc; border-radius: 8px; padding: 10px; min-height: 56mm; }
            .box h4 { margin: 0 0 8px; font-size: 12px; text-transform: uppercase; letter-spacing: .08em; color: #334155; }
            .content { font-size: 18px; line-height: 1.45; color: #0f172a; white-space: pre-line; font-weight: 600; }
            .footer { margin-top: 10mm; display: flex; justify-content: space-between; align-items: flex-end; }
            .barcodeBlock { min-width: 80mm; }
            .barcodeVisual { height: 16mm; width: 84mm; margin-bottom: 2mm; overflow: hidden; background: #fff; }
            .barcodeVisual svg { width: 100%; height: 100%; display: block; }
            .trackingLabel { font-size: 12px; color: #334155; margin-bottom: 1mm; }
            .barcode { font-family: monospace; font-size: 22px; letter-spacing: 2px; color: #111827; }
            .brand { text-align: right; color: #334155; font-size: 12px; }
            .brandLogo { width: 32mm; height: auto; margin: 0 0 2mm auto; display: block; }
            .brand strong { display: block; font-size: 20px; color: #111827; letter-spacing: .02em; }
            @media print {
              body { background: #fff; }
              .page { margin: 0; border: none; width: auto; min-height: auto; padding: 8mm; }
            }
          </style>
        </head>
        <body>
          <div class="page">
            <div class="top">
              <div class="senderInfo">
                ...
                <p>Aracı Firma: mektuplas.com</p>
                <p>Şirket Adı: EHM DİJİTAL ÇÖZÜMLER</p>
                <p>YAZILIM VE TICARET LİMİTED ŞİRKETİ</p>
                <p>Web Sitesi: mektuplas.com</p>
                <p>İletişim: info@mektuplas.com</p>
              </div>
              <div class="stamp">
                <img src="${logoUrl}" alt="Pul" />
              </div>
            </div>
            <div class="grid">
              <div class="box">
                <h4>Gönderen</h4>
                <div class="content">${senderName}<br/>${senderAddress}${senderCity ? `<br/>${senderCity}` : ""}</div>
              </div>
              <div class="box">
                <h4>Alıcı</h4>
                <div class="content">${receiverBlock}</div>
              </div>
            </div>
            <div class="footer">
              <div class="barcodeBlock">
                <div class="barcodeVisual">${barcodeSvg}</div>
                <div class="trackingLabel">Takip No / Barkod</div>
                <div class="barcode">*${escapeHtml(orderRef)}*</div>
              </div>
              <div class="brand">
                <img src="${logoUrl}" alt="Mektuplaş" class="brandLogo" />
                <strong>Mektuplaş</strong>
      ...
      setLetters((prev) =>
        prev.map((l) =>
          l.id === selectedLetter.id
            ? { ...l, trackingCode, status: "SHIPPED" }
            : l,
        ),
      );
      setSelectedLetter((prev) =>
        prev ? { ...prev, trackingCode, status: "SHIPPED" } : null,
      );
  ...
  const handleExportZip = async (letter: Letter) => {
    ...
    try {
      ...
      // 2. Add summary
      const giftsList =
        letter.data.extras.gifts && letter.data.extras.gifts.length > 0
          ? letter.data.extras.gifts
              .map((g: any) => `${g.name} (x${g.quantity})`)
              .join(", ")
          : "Yok";

      const summary = `
Siparis No: ${letter.data?.orderNumber || letter.id}
...
Tarih: ${new Date(letter.createdAt).toLocaleDateString("tr-TR")}
      ...
      photos.forEach((p: any) => {
        if (p.url)
          mediaItems.push({ url: p.url, name: `resim_${photoCount++}` });
      });

      let docCount = 1;
      docs.forEach((d: any) => {
        if (d.url) mediaItems.push({ url: d.url, name: `belge_${docCount++}` });
      });
      ...
      postcardIds.forEach((id: string) => {
        for (const cat of postcardCategories) {
          ...
          if (found) {
            mediaItems.push({
              url: found.image,
              name: `kartpostal_${postcardCount++}`,
            });
            break;
          }
        }
      ...
      for (let i = 0; i < mediaItems.length; i++) {
        try {
          ...
          try {
            ...
            if (
              pathExt &&
              ["jpg", "jpeg", "png", "pdf", "heic"].includes(pathExt)
            ) {
              ext = pathExt;
            }
          } catch (e) {
        ...
        {filteredLetters.length === 0 && (
          <div className="col-span-full py-20 text-center bg-white rounded-xl border border-dashed border-slate-300">
            ...
            <p className="text-slate-500">
              Arama kriterlerinize uygun mektup kaydı bulunmuyor.
            </p>
          </div>
        )}
      </div>
              ...
              <div
                className={`px-6 py-4 flex items-center justify-between ${
                  selectedLetter.status === "PAID"
                    ...
                    : selectedLetter.status === "PREPARING"
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8">
                      ...
                      <div className="bg-slate-50 p-4 rounded-xl space-y-3">
                        <div className="flex justify-between items-center text-sm">
                          ...
                          <span className="font-bold text-slate-900">
                            {selectedLetter.data?.orderNumber ||
                              `#${selectedLetter.id.toUpperCase()}`}
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-slate-500">Müşteri:</span>
                          <span className="font-medium text-slate-900">
                            {selectedLetter.user.email}
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-slate-500">Tutar:</span>
                          <span className="font-bold text-slate-900">
                            {selectedLetter.totalAmount} TL
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="bg-slate-50 p-4 rounded-xl grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <span className="text-sm font-bold text-slate-900">
                            {selectedLetter.data.letter.envelopeColor}
                          </span>
                        </div>
                        <div>
                          <span className="text-xs text-slate-500 block">
                          ...
                          <span className="text-sm font-bold text-slate-900">
                            {selectedLetter.data.letter.paperColor}
                          </span>
                        </div>
                        <div>
                          <span className="text-xs text-slate-500 block">
                          ...
                          <span className="text-sm font-bold text-slate-900">
                            {selectedLetter.data.extras.scent}
                          </span>
                        </div>
                        <div>
                          <span className="text-xs text-slate-500 block">
                          ...
                          <span className="text-sm font-bold text-slate-900">
                            {selectedLetter.data.extras.deliveryDate}
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {selectedLetter.data.extras.postcards?.length > 0 && (
                          <span className="px-3 py-1 bg-purple-50 text-purple-700 text-xs font-bold rounded-full border border-purple-100">
                            {selectedLetter.data.extras.postcards.length}{" "}
                            Kartpostal
                          </span>
                        )}
                      ...
                      <div>
                        ...
                        <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm">
                          <p className="font-bold text-slate-900 mb-1">
                            {selectedLetter.data.address.senderName}
                          </p>
                          <p className="text-sm text-slate-600 leading-relaxed">
                            {selectedLetter.data.address.senderAddress}
                          </p>
                          <p className="text-sm text-slate-900 font-bold mt-2">
                            {selectedLetter.data.address.senderCity}
                          </p>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="bg-white border border-blue-200 p-4 rounded-xl shadow-sm relative">
                          ...
                          <p className="font-bold text-slate-900 mb-1">
                            {selectedLetter.data.address.receiverName}
                          </p>
                          <p className="text-sm text-slate-900 font-bold mt-2">
                            {selectedLetter.data.address.receiverCity}
                          </p>
                          {selectedLetter.data.address.isPrison ? (
                            <>
                              <p className="text-sm font-bold text-slate-800">
                                {selectedLetter.data.address.prisonName}
                              </p>
                              {selectedLetter.data.address.fatherName && (
                                <p className="text-xs text-slate-600">
                                  Baba Adı:{" "}
                                  {selectedLetter.data.address.fatherName}
                                </p>
                              )}
                              {selectedLetter.data.address.wardNumber && (
                                <p className="text-xs text-slate-600">
                                  Koğuş:{" "}
                                  {selectedLetter.data.address.wardNumber}
                                </p>
                              ...
                              <p className="text-sm text-slate-600 leading-relaxed mt-1">
                                {selectedLetter.data.address.receiverAddress}
                              </p>
                            </>
                          ) : (
                            <p className="text-sm text-slate-600 leading-relaxed">
                        ...
                        {selectedLetter.status === "PAID" && (
                          <button
                            onClick={() => {
                              if (
                                window.confirm(
                                  "Bu mektubu hazırlamaya başlamak istediğinize emin misiniz? (Kullanıcıya bildirim e-postası gönderilecektir)",
                                )
                              ) {
                                handleUpdateStatus(
                                  selectedLetter.id,
                ...
                <div className="mt-12 pt-8 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <button
                      onClick={() => handlePrintEnvelope(selectedLetter)}
                      className="flex items-center justify-center gap-2 p-4 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl transition-all group"
                    >
                      <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-slate-400 group-hover:text-emerald-600 shadow-sm transition-colors">
                        <Printer size={20} />
                      </div>
                      <div className="text-left">
                        <span className="block text-sm font-bold text-slate-900">
                  ...
                  <div className="bg-slate-50 p-8 rounded-2xl border border-slate-100 font-serif text-slate-800 leading-relaxed max-h-96 overflow-y-auto print:max-h-none print:p-0 print:bg-white print:border-none shadow-inner prose prose-slate max-w-none">
                    <div
                      dangerouslySetInnerHTML={{
                        __html: selectedLetter.data.letter.content,
                      }}
                    />
                  </div>
                  ...
                  {selectedLetter.data.extras?.wantReplyInInbox && (
                    <div className="mt-6 p-5 bg-teal-50 border border-teal-200 rounded-xl">
                      ...
                      <p className="text-xs text-teal-700 leading-relaxed">
                        Bu mektubu yazan kişi, cevap mektuplarını aşağıdaki
                        adrese göndermenizi rica etmektedir. Firmamız
                        mektubunuzu göndericiye dijital olarak iletecektir.
                      </p>
                      <div className="mt-3 p-3 bg-white rounded-lg border border-teal-100">
                        <p className="text-xs font-bold text-teal-900">
                          Cevap Adresi:
                        </p>
                        <p className="text-xs text-teal-700 whitespace-pre-line">
                          {companyAddress || "Henüz adres girilmemiş."}
                        </p>
                      </div>
                      <p className="text-[10px] text-teal-600 mt-2 italic">
                        ⚠️ Bu bilgi mektubun sonuna otomatik olarak
                        eklenecektir.
                      </p>
                    </div>
                  )}
